 ...
 * @since 1.3
 */
public abstract class AbstractQuicksearchSourceViewerBAK extends SourceViewer {

	private IPropertyChangeListener fPropertyChangeListener = this::handlePreferenceStoreChanged;
	...
	public AbstractQuicksearchSourceViewerBAK(Composite parent) {
		this(parent,null, false, SWT.H_SCROLL | SWT.V_SCROLL | SWT.MULTI | SWT.BORDER | SWT.FULL_SELECTION);
	}

	public AbstractQuicksearchSourceViewerBAK(Composite parent, IOverviewRuler overviewRuler,
			boolean showAnnotationsOverview, int styles) {
		super(parent, new CompositeRuler(), overviewRuler, showAnnotationsOverview, styles | SWT.READ_ONLY);
		addVerticalRulerColumn(fLineNumberRulerColumn = new LineNumberRulerColumn());
	}

	protected void initialize() {
		initialize(EditorsUI.getPreferenceStore());
	}
	...
	protected void initialize(IPreferenceStore store) {
		...
		updateContributedRulerColumns((CompositeRuler) getVerticalRuler());
	...
	protected void initializeColors() {
		...
		if (store != null) {
			...
			getTextWidget().setSelectionForeground(color);
			...
			getTextWidget().setSelectionBackground(color);

			fLineNumberRulerColumn.setBackground(getTextWidget().getBackground());
	...
	private void setFont(Font font) {
		if (getDocument() != null) {
			...
			((CompositeRuler) getVerticalRuler()).setFont(font);
